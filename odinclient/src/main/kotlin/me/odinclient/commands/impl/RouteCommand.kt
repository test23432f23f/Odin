...
val RouteCommand = commodore("route") {
    ...
    literal("add").runs { subId: Int, type: String, yOffset: Double ->
                ...
                val route = RoutesManager.Route(
                    ...
                    if(AutoRouteUtils.currentRoom != null) AutoRouteUtils.currentRoom!!.getRelativeCoords(multiply(mop.hitVec, 1.00).addVector((mop.hitVec.xCoord - Math.ceil(mop.hitVec.xCoord).toDouble()) / 2.0, Math.ceil(mop.hitVec.yCoord).toDouble() - mop.hitVec.yCoord, (mop.hitVec.zCoord - Math.ceil(mop.hitVec.zCoord).toDouble()) / 2.0)) else multiply(mop.hitVec, 1.00).addVector(0.0, Math.ceil(mop.hitVec.yCoord) - mop.hitVec.yCoord, 0.0),
