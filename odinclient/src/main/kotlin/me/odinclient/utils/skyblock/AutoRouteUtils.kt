...
class AutoRouteUtils : Module(
            ...
            for (id in RoutesManager.instance.loadedRoutes[roomId]!!.keys) {
                ...
                for (i in routes.indices) {
                    ...
                    if ((Vec3(
                        ...
                        val dy: Int = Math.round((getYaw(route.yaw, currentRoom!!.getRealCoords(nextRoute.pos)) - 180.0f - route.yaw).toFloat())
   ...
   companion object
   {
       ...
       fun getYaw(yaw: Float, end: Vec3): Float {
            ...
            Minecraft.getMinecraft().thePlayer.posZ).toFloat() * 180.0f / Math.PI - 90.0F).toFloat() - yaw
        )
    }
